...
const uploadChunkValidators = [
  ...
  body('chunk_index')
    .isInt({ min: 0 })
    .withMessage('chunk_index must be a non-negative integer'),
  body('total_chunks')
    .isInt({ min: 1 })
    .withMessage('total_chunks must be at least 1'),
...
router.post('/upload', authenticate, upload.single('data'), uploadChunkValidators, fileController.uploadChunk);
